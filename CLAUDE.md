...
## On the phone (home Wi-Fi)

Added 2026-09-14. The same server and the same page serve the phone. There is no
separate mobile app and no second codebase, so a phone can never disagree
with the laptop. The "full cloud" plan below is still the long-term target. This
setup keeps the frontend on the `/api/*` shape so it survives that move.

**Login (`app/auth.py`), rules that are load-bearing:**

- **Loopback is exempt, everything else needs the cookie.** The laptop never
  sees a login page. The check uses the socket peer (`request.client.host`), and
  uvicorn runs with `--no-proxy-headers`, so a forged `X-Forwarded-For:
  127.0.0.1` cannot pass as local. This was tested.
- **Fail closed.** With no `data/auth.json` every non-local request gets 403, and
  `run-lan.bat` refuses to start. A LAN server with no password is never one
  keystroke away.
- scrypt (n=2^14) password hash. The session is `issued.HMAC(secret, issued)`,
  httponly, SameSite=Lax, 30 days. Setting a new password rotates the secret, so
  it signs out every device.
- 5 wrong passwords from one IP lock it out for 30 s, doubling each time up to 1 h.
- `_PUBLIC` lets only the login page, the stylesheet, the manifest and the icons
  through unauthenticated. `app.js` is not public.
- **Claude never sets the password.** The user runs `set-password.bat`.

**Layout: one stylesheet, one breakpoint (`max-width: 700px`).**

- The nav becomes a fixed bottom bar with icons (`.ti`/`.ts`, hidden on desktop).
- Tables that cannot fit get a card twin rendered from the same data:
  `.trade-cards` for the trades list, `.ledger-cards` for a trade's ledger. CSS
  picks which one shows.
- Dialogs go full screen with sticky actions.
- Inputs are 16px, because below that iOS zooms on focus.
- Tap targets are at least 40px.
- Inline row editing does not fit a phone, so `editEntryRow` opens
  `#entry-dialog` instead (`narrow()`). Both save through `saveEntry`.
- **Charts are redrawn at phone geometry, not scaled down.** A 760-wide viewBox
  squeezed into 360px put the text at ~6px. `scoreGeom()` returns a 400-wide
  canvas when `narrow()` is true, and the cumulative chart does the same.
- **Tooltips on touch are sticky.** A tap shows one, `pointerleave` from a touch
  is ignored, and `dismissOnTapOutside` hides it. Otherwise the tooltip would
  vanish as the finger lifts.
- Trade bands are clipped to the plot area (`plotTop..plotBottom`). They used
  `y(100)`/`y(0)`, which lie outside the 20-90 axis and spilled over the labels.

**Home screen:** `static/manifest.json` plus icons generated by
`tools/make_icons.py`, which uses the standard library only. **No service worker:**
browsers only allow one over HTTPS or localhost, and plain-HTTP LAN is neither.
So the app works only while the laptop is on and reachable.

**Windows network gotcha:** a Wi-Fi network classified as *Public* blocks inbound
port 8000 even with an allow rule scoped to Private. Set the profile to Private
first.
...
## Build / run
    ...
    set-password.bat  set/change the phone login password (run by the user)
    run-lan.bat       serves 0.0.0.0:8000 for the phone; prints the address
    .venv\Scripts\python.exe tests\test_core.py     offline checks
